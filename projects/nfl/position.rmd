...
```{r packages}
library(pacman)
p_load(rvest,tidyverse,data.table,janitor,cowplot,rvest,RCurl,ggimage)
```

Now let's scrape some data on NFL salaries for 2019, which we will combine with win-loss data from wikipedia. Let's focus on regular season games from the 2019 season to start. Our three dataframes will be one denoting positional spending, one showing the standings from the regular season, and one including the logos of each team from wikipedia.
...
url.logo <- getURL(
  "https://raw.githubusercontent.com/statsbylopez/BlogPosts/master/nfl_teamlogos.csv"
  )
...
df.logos <- read.csv(text = url.logo) %>% 
  ...
  tibble()
df.logos$url[32] <-
  "https://upload.wikimedia.org/wikipedia/commons/thumb/7/72/Washington_football_team_wlogo.svg/800px-Washington_football_team_wlogo.svg.png"
...
Next, lets get a barchart of positional data for each team. Notice that the data is already in long format, so we can immediately jump into ggplot2.
...
Next, we will create an x-axis depicting each teams logo. This is implemented through a brute force google image search for each team. Suboptimal? Perhaps, but is the best I got for now!
...
However, after some testing I've failed to find significant evidence of significant correlation between positional spending of any kind and win-loss percentage. Also, some light literature review seems to argue that Offense and Defense are each generally equivalent in their statistical effect on team success, implying that differences in positional spending effectiveness might depend on the individual team's approach to offense or defense. 
...
p2 <- naruki_is_santa %>%
  ...
  mutate(
    unit = ifelse(
      position %in% c("QB","RB/FB","WR","TE","OL"),
      "Offense",
      ifelse(position == "K/P/LS","Special","Defense")
    )
  ) %>%
  ggplot() +
  geom_bar(
  ...
  ylab("Salary Expenditures")
ggdraw(insert_xaxis_grob(p2,pimage,position = "bottom"))
```


For example, the Green Bay Packers went 13-3, while the New England Patriots went 12-4. However, Green Bay spent over 100 million USD on their offense, leaving 60 million USD for defense and special teams. However, the patriots spent their money distributed more evenly between their offense and defense, with 79 million and 75 million USD respectively. 
...
p3 <- naruki_is_santa %>% 
  ...
  geom_image(mapping = aes(image = url),size = .05) +
  geom_smooth(method = "lm") +
  ylab("Win-Loss Percentage") +
  xlab("Point Differential") +
  labs(
    title = "NFL Team Record 2019", 
    subtitle = "Predicted by Point Differential"
    ) +
  theme(
  ...
  geom_text(
    ...
    label = paste(
      "y = ",
      round(lm1$coefficients[1],digits = 4),
      " + ",
      round(lm1$coefficients[2],digits = 4),
      "x"
      )
  ) +
  geom_text(
    ...
    label = paste(
      "R-squared:",
      round(summary(lm1)$r.squared,digits = 4)
      )
  )
p3
```